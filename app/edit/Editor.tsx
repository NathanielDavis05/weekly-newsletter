...
import { defaultHeader, RESERVED_SLUGS, visualDocument } from "../content/visual";
import { formatDate, parseNewsletterDate } from "../content/dates";
...
const builtInPages: Array<{ id: VisualPageId; label: string }> = [{ id: "home", label: "Home" }, { id: "training", label: "Training" }, { id: "results", label: "Results" }];
type IssueTarget = "draft" | "live";
type Template = { id: string; kind: Exclude<VisualBlockKind, "native">; label: string; icon: string; title?: string; body?: string; buttonLabel?: string; href?: string; imageUrl?: string; style?: BlockStyle; accentBar?: VisualBlock["accentBar"]; decoration?: VisualBlock["decoration"]; variant?: VisualBlock["variant"]; eyebrow?: string; micro?: string; cardIcon?: string };
const templates: Template[] = [
  ...
  { id: "card", kind: "container", label: "Basic card", icon: "▣", title: "Important update", body: "Add supporting details here.", style: { background: "#fffdf8", borderColor: "#ddd3c4", borderWidth: 1, borderRadius: 18, paddingTop: 20, paddingRight: 20, paddingBottom: 20, paddingLeft: 20, phone: { width: 100 }, desktop: { width: 70 } } },
  { id: "action-card", kind: "container", label: "Action card", icon: "↗", title: "Your feedback matters", body: "Share your thoughts with the team in a few quick minutes.", buttonLabel: "Take action", href: "https://", accentBar: { enabled: true, color: "#d80d37", width: 58, height: 5, inset: 0 }, style: { background: "#fffdf8", borderColor: "#d80d37", borderWidth: 1, borderRadius: 18, paddingTop: 24, paddingRight: 24, paddingBottom: 24, paddingLeft: 24, shadow: "soft", phone: { width: 100 }, desktop: { width: 72 } } },
  { id: "survey-card", kind: "container", label: "Survey action card", icon: "☑", variant: "feature-card", eyebrow: "TEAM FEEDBACK · OPEN THROUGH AUGUST 17", title: "Monthly Team Member Engagement Survey", body: "Your feedback helps us build a better workplace. Respond anonymously, or leave your name at the end if you’d like follow-up.", micro: "Open through August 17", cardIcon: "☑", buttonLabel: "Take the survey", href: "https://www.surveymonkey.com/r/QKB5FD2", accentBar: { enabled: true, color: "#d80d37", width: 100, height: 5, inset: 0 }, style: { background: "#fffdf8", borderColor: "#ddd3c4", borderWidth: 1, borderRadius: 18, paddingTop: 28, paddingRight: 28, paddingBottom: 28, paddingLeft: 28, shadow: "soft", phone: { width: 100 }, desktop: { width: 100 } } },
  { id: "section-box", kind: "container", label: "Section box", icon: "▦", title: "New section", body: "Add the details for this section.", style: { background: "#fffdf8", borderColor: "#e3d7cf", borderWidth: 1, borderRadius: 14, paddingTop: 22, paddingRight: 22, paddingBottom: 22, paddingLeft: 22, phone: { width: 100 }, desktop: { width: 78 } } },
  ...
  { id: "shape", kind: "shape", label: "Decorative shape", icon: "✦", decoration: { shape: "star", color: "#d80d37", size: 52, x: 0, y: 0, opacity: 100, rotation: 0 }, style: { phone: { width: 100, minHeight: 58 }, desktop: { width: 18, minHeight: 58 } } },
...
const makeItem = (template: Template): VisualBlock => ({ id: uid(), kind: template.kind, label: template.label, title: template.title, body: template.body, buttonLabel: template.buttonLabel, href: template.href, imageUrl: template.imageUrl, alt: template.kind === "image" ? "Newsletter image" : undefined, variant: template.variant, eyebrow: template.eyebrow, micro: template.micro, icon: template.cardIcon, accentBar: template.accentBar ? structuredClone(template.accentBar) : undefined, decoration: template.decoration ? structuredClone(template.decoration) : undefined, style: template.style ? structuredClone(template.style) : undefined });

/** A human week label stays tied to the newsletter's real dated hero line. */
function issueWeekLabel(content: NewsletterContent): string {
  const parsed = parseNewsletterDate(content.home.hero.kicker);
  if (!parsed) return content.home.hero.kicker.trim() || "Undated issue";
  const end = new Date(parsed.start.getTime() + 6 * 86_400_000);
  const start = formatDate(parsed.start, "long", false);
  return parsed.start.getMonth() === end.getMonth()
    ? `Week of ${start}–${end.getDate()}, ${end.getFullYear()}`
    : `Week of ${start}–${formatDate(end, "long")}`;
}

function issueStart(content: NewsletterContent): number | null {
  return parseNewsletterDate(content.home.hero.kicker)?.start.getTime() ?? null;
}
...
function IconPositionControls({ name, device, layout, patch, background = false }: { name: string; device: "phone" | "desktop"; layout?: ResponsiveLayout; patch: (patch: Partial<ResponsiveLayout>) => void; background?: boolean }) {
  return <details className="inspector-section" open><summary>{name} icon · {device}</summary><div className="inspector-grid"><NumberField label="Move icon X" value={layout?.iconX ?? 0} min={-80} max={80} onCommit={(value) => patch({ iconX: value ?? 0 })} /><NumberField label="Move icon Y" value={layout?.iconY ?? 0} min={-80} max={80} onCommit={(value) => patch({ iconY: value ?? 0 })} /></div><NumberField label="Icon size %" value={layout?.iconScale ?? 100} min={40} max={180} onCommit={(value) => patch({ iconScale: value ?? 100 })} />{background ? <><p className="inspector-device-note">Circle background</p><div className="inspector-grid"><NumberField label="Move background X" value={layout?.iconBackgroundX ?? 0} min={-80} max={80} onCommit={(value) => patch({ iconBackgroundX: value ?? 0 })} /><NumberField label="Move background Y" value={layout?.iconBackgroundY ?? 0} min={-80} max={80} onCommit={(value) => patch({ iconBackgroundY: value ?? 0 })} /></div><NumberField label="Background size %" value={layout?.iconBackgroundScale ?? 100} min={40} max={180} onCommit={(value) => patch({ iconBackgroundScale: value ?? 100 })} /></> : null}<div className="inspector-actions"><button type="button" onClick={() => patch({ iconX: 0, iconY: 0, iconScale: 100, ...(background ? { iconBackgroundX: 0, iconBackgroundY: 0, iconBackgroundScale: 100 } : {}) })}>Reset icon{background ? " and background" : ""}</button></div><p className="inspector-device-note">{background ? "Move and resize the icon and its circle separately. " : "Move and resize this icon exactly where you want it. "}Phone and desktop can be different.</p></details>;
}

function NativeEditor({ id, content, change, device, iconLayout, patchIcon }: { id: string; content: NewsletterContent; change: (mutator: (draft: NewsletterContent) => void) => void; device: "phone" | "desktop"; iconLayout?: ResponsiveLayout; patchIcon: (patch: Partial<ResponsiveLayout>) => void }) {
  ...
  if (id === "home-action") { const c = h.overview.actionCard; return <>{text("Label", c.label, (d, v) => { d.home.overview.actionCard.label = v; })}{text("Heading", c.heading, (d, v) => { d.home.overview.actionCard.heading = v; })}{text("Body", c.bodyPrefix, (d, v) => { d.home.overview.actionCard.bodyPrefix = v; }, true)}{text("Body emphasis", c.bodyEmphasis, (d, v) => { d.home.overview.actionCard.bodyEmphasis = v; })}{text("Micro note", c.micro, (d, v) => { d.home.overview.actionCard.micro = v; })}{text("Link text", c.linkLabel, (d, v) => { d.home.overview.actionCard.linkLabel = v; })}{text("Link", c.linkHref, (d, v) => { d.home.overview.actionCard.linkHref = v; })}<IconPositionControls name="TV" device={device} layout={iconLayout} patch={patchIcon} background /></>; }
  if (id === "home-event" || id === "home-recognition-link") { const key: "eventCard" | "recognitionCard" = id === "home-event" ? "eventCard" : "recognitionCard"; const card = h.overview[key]; return <>{text("Kicker", card.kicker, (d, v) => { d.home.overview[key].kicker = v; })}{text("Title", card.title, (d, v) => { d.home.overview[key].title = v; })}{text("Detail", card.detail, (d, v) => { d.home.overview[key].detail = v; })}{text("Link", card.href, (d, v) => { d.home.overview[key].href = v; })}{id === "home-event" ? <IconPositionControls name="Calendar" device={device} layout={iconLayout} patch={patchIcon} background /> : null}</>; }
  ...
  if (id === "home-birthday") { const birthday = h.recognition.birthday; const entries = birthday.entries ?? [{ name: birthday.name, date: birthday.date }]; const updateBirthdays = (mutator: (list: { name: string; date: string }[]) => void) => change((draft) => { const target = draft.home.recognition.birthday; const next = structuredClone(target.entries ?? [{ name: target.name, date: target.date }]); mutator(next); target.entries = next; const first = next[0] ?? { name: "", date: "" }; target.name = first.name; target.date = first.date; }); const moveBirthday = (index: number, direction: number) => updateBirthdays((list) => { const next = index + direction; if (next < 0 || next >= list.length) return; [list[index], list[next]] = [list[next], list[index]]; }); return <>{text("Kicker", birthday.kicker, (d, v) => { d.home.recognition.birthday.kicker = v; })}<div className="inspector-list">{entries.map((entry, index) => <ListItemFrame key={index} index={index} count={entries.length} onMove={(direction) => moveBirthday(index, direction)} onRemove={() => updateBirthdays((list) => { list.splice(index, 1); })}><TextField label="Name" value={entry.name} onChange={(value) => updateBirthdays((list) => { list[index].name = value; })} /><TextField label="Date" value={entry.date} onChange={(value) => updateBirthdays((list) => { list[index].date = value; })} /></ListItemFrame>)}</div><button type="button" className="list-add" onClick={() => updateBirthdays((list) => { list.push({ name: "New teammate", date: "Month day" }); })}>+ Add birthday</button></>; }
  ...
  if (id === "home-referral") return <>{text("Referral heading", h.grow.referralStrong, (d, v) => { d.home.grow.referralStrong = v; })}{text("Referral detail", h.grow.referralRest, (d, v) => { d.home.grow.referralRest = v; }, true)}<IconPositionControls name="Present" device={device} layout={iconLayout} patch={patchIcon} background /></>;
...
function HeroInspector({ page, header, content, patch, patchDevice, upload, change }: { page: VisualPageId; header: HeaderStyle; content: NewsletterContent; patch: (next: Partial<HeaderStyle>) => void; patchDevice: (device: "phone" | "desktop", next: Partial<HeaderDeviceStyle>) => void; upload: (file: File) => void; change: (mutator: (draft: NewsletterContent) => void) => void }) {
  ...
  return <div className="editor-inspector-form"><div className="inspector-heading"><p className="visual-kicker">Hero</p><h2>{builtInPages.find((item) => item.id === page)?.label} header</h2></div>
...
/** Visual details shared by a basic card and the independent shape element. */
function DecorationControls({ item, patchItem }: { item: VisualBlock; patchItem: (id: string, patch: Partial<VisualBlock>) => void }) {
  const accent = item.accentBar ?? { enabled: false, color: "#d80d37", width: 54, height: 5, inset: 0 };
  const decoration = item.decoration ?? { shape: "none" as const, color: "#d80d37", size: 48, x: 0, y: 0, opacity: 100, rotation: 0 };
  const updateAccent = (patch: Partial<NonNullable<VisualBlock["accentBar"]>>) => patchItem(item.id, { accentBar: { ...accent, ...patch } });
  const updateDecoration = (patch: Partial<NonNullable<VisualBlock["decoration"]>>) => patchItem(item.id, { decoration: { ...decoration, ...patch } });
  return <details className="inspector-section" open>
    <summary>{item.kind === "shape" ? "Shape" : "Card details"}</summary>
    {item.kind === "container" ? <>
      <ToggleField label="Show a top accent bar" checked={accent.enabled} onChange={(enabled) => updateAccent({ enabled })} />
      {accent.enabled ? <><ColorField label="Accent color" value={accent.color} onChange={(color) => updateAccent({ color })} /><div className="inspector-grid"><NumberField label="Accent width %" value={accent.width} min={12} max={100} onCommit={(width) => updateAccent({ width })} /><NumberField label="Accent height" value={accent.height} min={2} max={20} onCommit={(height) => updateAccent({ height })} /></div><NumberField label="Accent inset" value={accent.inset} min={0} max={120} onCommit={(inset) => updateAccent({ inset })} /></> : null}
    </> : null}
    <label className="visual-control"><span>{item.kind === "shape" ? "Shape type" : "Decorative shape"}</span><select value={decoration.shape} onChange={(event) => updateDecoration({ shape: event.target.value as NonNullable<VisualBlock["decoration"]>["shape"] })}><option value="none">None</option><option value="circle">Circle</option><option value="star">Star</option><option value="sparkle">Sparkle</option><option value="diamond">Diamond</option></select></label>
    {decoration.shape !== "none" ? <><ColorField label="Shape color" value={decoration.color} onChange={(color) => updateDecoration({ color })} /><div className="inspector-grid"><NumberField label="Shape size" value={decoration.size} min={8} max={360} onCommit={(size) => updateDecoration({ size })} /><NumberField label="Opacity %" value={decoration.opacity} min={0} max={100} onCommit={(opacity) => updateDecoration({ opacity })} /></div><div className="inspector-grid"><NumberField label="Move shape X" value={decoration.x} min={-500} max={500} onCommit={(x) => updateDecoration({ x })} /><NumberField label="Move shape Y" value={decoration.y} min={-500} max={500} onCommit={(y) => updateDecoration({ y })} /></div><NumberField label="Rotate shape °" value={decoration.rotation} min={-180} max={180} onCommit={(rotation) => updateDecoration({ rotation })} /><p className="inspector-device-note">Shapes are decorative. Move the whole block with its drag handle; in a card, these X/Y controls place the shape exactly from the top-right corner.</p></> : <p className="inspector-note">Choose a shape to add the same kind of visual energy as the celebration card.</p>}
  </details>;
}
...
export function Editor({ initialDraft, initialPublished, initialRevision, userEmail }: { initialDraft: NewsletterContent; initialPublished: NewsletterContent; initialRevision: number; userEmail: string }) {
  ...
  const initialDraftContent = normalized(initialDraft); const initialPublishedContent = normalized(initialPublished);
  const [content, setContent] = useState<NewsletterContent>(() => initialDraftContent); const [saved, setSaved] = useState<NewsletterContent>(() => initialDraftContent); const [published, setPublished] = useState<NewsletterContent>(() => initialPublishedContent);
  const [issueTarget, setIssueTarget] = useState<IssueTarget>("draft");
  const draftBufferRef = useRef({ content: initialDraftContent, saved: initialDraftContent });
  const liveBufferRef = useRef({ content: initialPublishedContent, saved: initialPublishedContent });
  const issueTargetRef = useRef<IssueTarget>("draft");
  const [, setRevision] = useState(initialRevision); const revisionRef = useRef(initialRevision);
  const [page, setPage] = useState<VisualPageId>("home"); const [device, setDevice] = useState<"phone" | "desktop">("desktop");
  ...
  const pageTabs = useMemo(() => [...builtInPages, ...(document.customPages ?? []).map((item) => ({ id: item.id, label: item.title, slug: item.slug }))], [document.customPages]);
  const pageDocument = document.pages[page]; const selected = pageDocument.items.find((item) => item.id === selectedId) ?? null; const selectedRow = pageDocument.rows.find((row) => row.itemIds.includes(selectedId ?? ""));
  ...
  const draftWeekLabel = useMemo(() => issueWeekLabel(content), [content]);
  const publishedWeekLabel = useMemo(() => issueWeekLabel(published), [published]);
  const draftStart = useMemo(() => issueStart(content), [content]);
  const publishedStart = useMemo(() => issueStart(published), [published]);
  const isFutureDraft = draftStart !== null && publishedStart !== null && draftStart > publishedStart;
  ...
  const edit = useCallback((mutator: (draft: NewsletterContent) => void, options?: { label?: string; coalesceKey?: string }) => {
    ...
    if (issueTargetRef.current === "draft") draftBufferRef.current = { ...draftBufferRef.current, content: next };
    else {
      liveBufferRef.current = { ...liveBufferRef.current, content: next };
      setPublished(next);
    }
  ...
  const addTemplate = useCallback((templateId: string, dropAt?: { x: number; y: number }) => {
    const template = templates.find((item) => item.id === templateId); if (!template) return;
    const item = makeItem(template);
    updateVisual((doc) => { doc.pages[page].items.push(item); doc.pages[page].rows.push({ id: `${page}-row-${uid()}`, itemIds: [item.id], gap: 16, align: "stretch", keepColumnsOnPhone: false }); });
    setSelectedIds([item.id]); setSelectedTextFrame(null); setDrawer(null); setInspectorOpen(true);
    // A canvas drop should honour the pointer, not append a block and make the
    // manager hunt for it. After React paints the new block we turn the exact
    // screen delta into the persistent per-device nudge.
    if (dropAt) requestAnimationFrame(() => {
      const element = surfaceRef.current?.querySelector<HTMLElement>(`[data-item-id="${CSS.escape(item.id)}"]`);
      if (!element) return;
      const rect = element.getBoundingClientRect();
      patchLayout(item.id, {
        nudgeX: Math.round(dropAt.x - rect.left - rect.width / 2),
        nudgeY: Math.round(dropAt.y - rect.top - Math.min(rect.height, 80) / 2),
      });
    });
  }, [page, patchLayout, updateVisual]);
  ...
  const undo = useCallback(() => { const state = history.undo(); if (state) { contentRef.current = state; if (issueTargetRef.current === "draft") draftBufferRef.current = { ...draftBufferRef.current, content: state }; else { liveBufferRef.current = { ...liveBufferRef.current, content: state }; setPublished(state); } setContent(state); } }, [history]);
  const redo = useCallback(() => { const state = history.redo(); if (state) { contentRef.current = state; if (issueTargetRef.current === "draft") draftBufferRef.current = { ...draftBufferRef.current, content: state }; else { liveBufferRef.current = { ...liveBufferRef.current, content: state }; setPublished(state); } setContent(state); } }, [history]);
  ...
  const acceptSaved = useCallback((data: ApiData, snapshot: NewsletterContent, target: IssueTarget) => {
    const next = normalized(target === "live" ? data.published ?? snapshot : data.draft ?? snapshot);
    const nextRevision = data.revision ?? revisionRef.current;
    revisionRef.current = nextRevision; setRevision(nextRevision);
    if (target === "draft") draftBufferRef.current = { content: next, saved: next };
    else { liveBufferRef.current = { content: next, saved: next }; setPublished(next); }
    if (issueTargetRef.current !== target) return;
    setSaved(next); setContent((current) => { const adopted = JSON.stringify(current) === JSON.stringify(snapshot) ? next : current; contentRef.current = adopted; return adopted; });
  }, []);
  ...
  const switchIssue = async (target: IssueTarget) => {
    const currentTarget = issueTargetRef.current;
    if (target === currentTarget || busy) return;
    if (dirty) {
      setBusy(true);
      try {
        await persist(structuredClone(contentRef.current), undefined, currentTarget);
        setStatus(currentTarget === "live" ? "Live changes saved" : "Draft saved");
      } catch (error) {
        const typed = error as Error & { status?: number };
        if (typed.status === 409) setConflict(true);
        setStatus(typed.message);
        return;
      } finally { setBusy(false); }
    }
    const next = target === "draft" ? draftBufferRef.current : liveBufferRef.current;
    issueTargetRef.current = target;
    contentRef.current = next.content;
    setIssueTarget(target); setContent(next.content); setSaved(next.saved);
    setPublished(liveBufferRef.current.content);
    setSelectedIds([]); setSelectedTextFrame(null); setSheetOpen(false);
    history.reset();
    setStatus(target === "live" ? "Editing the live issue" : "Editing next week's draft");
  };
  const publishNow = async () => { setBusy(true); try { await autosaveRef.current?.catch(() => undefined); let snapshot = structuredClone(content); if (JSON.stringify(snapshot) !== JSON.stringify(saved)) { const data = await api("/api/content", { method: "PUT", body: JSON.stringify({ content: snapshot, expectedRevision: revisionRef.current, target: "draft" }) }); acceptSaved(data, snapshot, "draft"); snapshot = normalized(data.draft ?? snapshot); } const result = await api("/api/content/publish", { method: "POST", body: JSON.stringify({ expectedRevision: revisionRef.current }) }); const nextRevision = result.revision ?? revisionRef.current; revisionRef.current = nextRevision; setRevision(nextRevision); const nextPublished = normalized(result.published ?? snapshot); setPublished(nextPublished); liveBufferRef.current = { content: nextPublished, saved: nextPublished }; setSaved(snapshot); draftBufferRef.current = { content: snapshot, saved: snapshot }; contentRef.current = snapshot; setContent(snapshot); setStatus("Published — live site updated"); setConflict(false); } catch (error) { const typed = error as Error & { status?: number }; if (typed.status === 409) setConflict(true); setStatus(typed.message); } finally { setBusy(false); } };
  ...
  const createCustomPage = useCallback(() => {
    const requestedTitle = globalThis.prompt("Name this new page", "New page");
    const title = requestedTitle?.trim().slice(0, 80);
    if (!title) return;
    const existingSlugs = new Set((document.customPages ?? []).map((item) => item.slug));
    const base = title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "new-page";
    let slug = base; let suffix = 2;
    while (RESERVED_SLUGS.has(slug) || existingSlugs.has(slug)) { slug = `${base}-${suffix}`; suffix += 1; }
    const id = `page-${uid()}` as VisualPageId;
    const headingId = `${id}-heading`; const introId = `${id}-intro`;
    updateVisual((doc) => {
      doc.customPages = [...(doc.customPages ?? []), { id, title, slug }];
      doc.pages[id] = {
        items: [
          { id: headingId, kind: "text", label: "Page heading", title, richTitle: richTextFromPlain(title), style: { fontSize: 42, fontWeight: 750, color: "#242126", phone: { width: 100 }, desktop: { width: 100 } } },
          { id: introId, kind: "text", label: "Introduction", body: "Add a short introduction to this page.", richBody: richTextFromPlain("Add a short introduction to this page."), style: { fontSize: 18, color: "#5d5659", phone: { width: 100 }, desktop: { width: 78 } } },
        ],
        rows: [
          { id: `${id}-row-heading`, itemIds: [headingId], gap: 16, align: "stretch", keepColumnsOnPhone: false },
          { id: `${id}-row-intro`, itemIds: [introId], gap: 16, align: "stretch", keepColumnsOnPhone: false },
        ],
        background: "#fbf7ef", contentWidth: 900, minHeight: 0, paddingTop: 30, paddingRight: 22, paddingBottom: 46, paddingLeft: 22, rowGap: 22,
      };
      doc.headers[id] = defaultHeader(id);
    }, { label: "Create page" });
    setPage(id); setSelectedIds([headingId]); setSelectedTextFrame(null); setDrawer("weekly"); setInspectorOpen(true);
    setStatus(`Created “${title}”`);
  }, [document.customPages, updateVisual]);
  ...
  const commands = useMemo<Command[]>(() => {
    const list: Command[] = [];
    for (const item of pageTabs) list.push({ id: `page-${item.id}`, group: "Pages", label: `Go to ${item.label} page`, action: { kind: "page", page: item.id } });
    ...
    const pageLabel = pageTabs.find((item) => item.id === page)?.label;
    ...
    return list;
  }, [document.looks, document.savedBlocks, page, pageDocument, pageTabs]);
  ...
  const dropTemplate = (event: ReactDragEvent<HTMLDivElement>) => { const id = event.dataTransfer.getData("application/x-newsletter-template"); if (id) { event.preventDefault(); addTemplate(id, { x: event.clientX, y: event.clientY }); } };
  const selectedStyle = selected?.style ?? {}; const currentLayout = selectedStyle[device] ?? {}; const heroSelected = (page === "home" || page === "training" || page === "results") && selectedId === `hero-${page}`;
  ...
  const activeCustomPage = document.customPages?.find((item) => item.id === page);
  const previewHref = activeCustomPage ? `/${activeCustomPage.slug}` : page === "home" ? "/edit/preview" : `/edit/preview/${page}`;
  ...
  return <div className="builder">
    ...
    <section className="issue-context" aria-label="Choose an issue to edit">
      <button type="button" className={`issue-context__card issue-context__draft${issueTarget === "draft" ? " is-active" : ""}`} aria-pressed={issueTarget === "draft"} onClick={() => void switchIssue("draft")} disabled={busy}>
        <span className="issue-context__icon" aria-hidden="true">✎</span><span className="issue-context__badge">Next week · draft</span><strong>{draftWeekLabel}</strong><p>{issueTarget === "draft" ? "Editing now — changes are private" : "Click to edit next week's draft"}</p><span className="issue-context__action">{issueTarget === "draft" ? "Currently editing" : "Edit draft →"}</span>
      </button>
      <button type="button" className={`issue-context__card issue-context__live${issueTarget === "live" ? " is-active" : ""}`} aria-pressed={issueTarget === "live"} onClick={() => void switchIssue("live")} disabled={busy}>
        <span className="issue-context__icon" aria-hidden="true">◉</span><span className="issue-context__badge">Live now</span><strong>{publishedWeekLabel}</strong><p>{issueTarget === "live" ? "Editing now — changes are live" : "Click to edit what readers see today"}</p><span className="issue-context__action">{issueTarget === "live" ? "Currently editing" : "Edit live issue →"}</span>
      </button>
    </section>
    ...
    <div className={`builder-workspace${inspectorOpen ? "" : " builder-workspace--wide"}${drawer ? " builder-workspace--drawer" : ""}`}>
      {drawer ? <aside className="builder-drawer"><nav className="editor-page-switcher" aria-label="Choose a newsletter page">
        <span>Editing page</span>
        <div>{pageTabs.map((item) => <button key={item.id} type="button" aria-current={page === item.id ? "page" : undefined} className={page === item.id ? "is-active" : ""} onClick={() => changePage(item.id)}>{item.label}</button>)}<button type="button" className="editor-page-switcher__add" onClick={createCustomPage} aria-label="Add a page">＋</button></div>
        <button type="button" className="editor-add-section" onClick={() => addTemplate("section-box")}>＋ Add section box</button>
      </nav>{drawer === "media" ? <MediaPanel
      ...
      /> : drawer === "weekly" ? <WeeklyMode
        ...
        draftLabel={draftWeekLabel}
        publishedLabel={publishedWeekLabel}
        ...
        onUndoNextIssue={undo}
        onSwitchIssue={(target) => void switchIssue(target)}
      ...
      {(isMobile ? sheetOpen : inspectorOpen) ? <aside className="builder-inspector">{isMobile ? <button type="button" className="inspector-close" onClick={() => setSheetOpen(false)} aria-label="Close panel"><span aria-hidden="true" /></button> : null}{selectedIds.length > 1 ? <div className="multi-actions">
        ...
        </div> : selectedTextFrame ? <TextFrameInspector label={heroSelected ? "Hero" : selected?.label ?? "Text"} frameKey={selectedTextFrame} style={selectedFrameStyle} layout={selectedFrameLayout} device={device} patch={(patch) => patchTextFrame(selectedTextFrame, patch)} patchLayout={(patch) => patchTextFrameLayout(selectedTextFrame, patch)} reset={() => resetTextFrame(selectedTextFrame)} back={() => setSelectedTextFrame(null)} /> : heroSelected ? <HeroInspector page={page} header={document.headers[page]} content={content} patch={patchHeader} patchDevice={patchHeaderDevice} upload={(file) => void upload(file, true)} change={edit} /> : selected ? <div className="editor-inspector-form"><div className="inspector-heading"><p className="visual-kicker">Selected item</p><h2>{selected.label}</h2></div>{selected.kind !== "native" ? <><label className="visual-control"><span>Label</span><input value={selected.label} onChange={(event) => patchItem(selected.id, { label: event.target.value })} /></label>{selected.kind === "text" || selected.kind === "container" || selected.kind === "button" || selected.kind === "subsection" ? <label className="visual-control"><span>{selected.kind === "button" ? "Button text" : selected.kind === "subsection" ? "Subsection text" : "Heading"}</span><input value={selected.title ?? ""} onChange={(event) => patchItem(selected.id, { title: event.target.value })} /></label> : null}{selected.kind === "text" || selected.kind === "container" ? <label className="visual-control"><span>Text</span><textarea value={selected.body ?? ""} onChange={(event) => patchItem(selected.id, { body: event.target.value })} /></label> : null}{selected.kind === "container" ? <><label className="visual-switch"><input type="checkbox" checked={selected.buttonLabel != null} onChange={(event) => patchItem(selected.id, event.target.checked ? { buttonLabel: "Take action", href: selected.href || "https://" } : { buttonLabel: undefined, href: undefined })} /> Include a linked button</label>{selected.buttonLabel != null ? <><label className="visual-control"><span>Button text</span><input value={selected.buttonLabel} onChange={(event) => patchItem(selected.id, { buttonLabel: event.target.value })} /></label><label className="visual-control"><span>Link</span><input value={selected.href ?? ""} onChange={(event) => patchItem(selected.id, { href: event.target.value })} /></label></> : null}</> : null}{selected.kind === "button" ? <label className="visual-control"><span>Link</span><input value={selected.href ?? ""} onChange={(event) => patchItem(selected.id, { href: event.target.value })} /></label> : null}{selected.kind === "image" ? <><label className="visual-control"><span>Image URL</span><input value={selected.imageUrl ?? ""} onChange={(event) => patchItem(selected.id, { imageUrl: event.target.value })} /></label><label className="visual-upload"><span>Upload image</span><input type="file" accept="image/*" onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); }} /></label></> : null}</> : <p className="inspector-note">This newsletter item keeps its existing structured copy. Its layout and appearance use the same renderer on the canvas and live site.</p>}
        {selected.kind === "native" ? <NativeEditor id={selected.nativeId ?? selected.id} content={content} change={edit} device={device} iconLayout={currentLayout} patchIcon={(patch) => patchLayout(selected.id, patch)} /> : null}
        {selected.kind === "container" || selected.kind === "shape" ? <DecorationControls item={selected} patchItem={patchItem} /> : null}
        {selected.kind === "text" || selected.kind === "container" ? <p className="inspector-note">To place a heading or paragraph exactly, click that text on the canvas. Its Text frame panel lets you drag it, resize it, or enter precise X/Y values.</p> : null}
        {!selected.attachedTo ? <AttachedSubsectionControls
          bar={attachedSubsection}
          device={device}
          onToggle={(enabled) => {
            if (enabled) createAttachedSubsection(selected.id, false);
            else if (attachedSubsection) removeAttachedSubsection(selected.id, attachedSubsection.id);
          }}
          patchItem={patchItem}
          patchLayout={patchLayout}
          onSelect={(id) => { setSelectedIds([id]); setSelectedTextFrame(null); setInspectorOpen(true); }}
        /> : null}
