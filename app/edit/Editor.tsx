...
function NativeEditor({ id, content, change, device, referralIcon, patchReferralIcon }: { id: string; content: NewsletterContent; change: (mutator: (draft: NewsletterContent) => void) => void; device: "phone" | "desktop"; referralIcon?: ResponsiveLayout; patchReferralIcon: (patch: Partial<ResponsiveLayout>) => void }) {
  ...
  if (id === "home-referral") return <>{text("Referral heading", h.grow.referralStrong, (d, v) => { d.home.grow.referralStrong = v; })}{text("Referral detail", h.grow.referralRest, (d, v) => { d.home.grow.referralRest = v; }, true)}<details className="inspector-section" open><summary>Present icon · {device}</summary><div className="inspector-grid"><NumberField label="Move icon X" value={referralIcon?.iconX ?? 0} min={-80} max={80} onCommit={(value) => patchReferralIcon({ iconX: value ?? 0 })} /><NumberField label="Move icon Y" value={referralIcon?.iconY ?? 0} min={-80} max={80} onCommit={(value) => patchReferralIcon({ iconY: value ?? 0 })} /></div><NumberField label="Icon size %" value={referralIcon?.iconScale ?? 100} min={40} max={180} onCommit={(value) => patchReferralIcon({ iconScale: value ?? 100 })} /><div className="inspector-actions"><button type="button" onClick={() => patchReferralIcon({ iconX: 0, iconY: 0, iconScale: 100 })}>Center icon</button></div><p className="inspector-device-note">Move and resize the present inside the circle. Phone and desktop positions can be different.</p></details></>;
  ...
  return <div className="builder">
    ...
    <div className={`builder-workspace${inspectorOpen ? "" : " builder-workspace--wide"}`}>
      ...
      {(isMobile ? sheetOpen : inspectorOpen) ? <aside className="builder-inspector">{isMobile ? <button type="button" className="inspector-close" onClick={() => setSheetOpen(false)} aria-label="Close panel"><span aria-hidden="true" /></button> : null}{selectedIds.length > 1 ? <div className="multi-actions">
        ...
        {selected.kind === "native" ? <NativeEditor id={selected.nativeId ?? selected.id} content={content} change={edit} device={device} referralIcon={currentLayout} patchReferralIcon={(patch) => patchLayout(selected.id, patch)} /> : null}
