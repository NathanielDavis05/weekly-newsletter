...
import type { BlockStyle, HeaderDeviceStyle, HeaderStyle, Look, NewsletterContent, ResponsiveLayout, SavedBlock, VisualBlock, VisualBlockKind, VisualPageId } from "../content/types";
...
function NativeEditor({ id, content, change }: { id: string; content: NewsletterContent; change: (mutator: (draft: NewsletterContent) => void) => void }) {
  ...
  if (id === "home-scorecard") { const s = h.scorecard; return <>{text("Eyebrow", s.eyebrow, (d, v) => { d.home.scorecard.eyebrow = v; })}{text("Heading", s.heading, (d, v) => { d.home.scorecard.heading = v; })}{text("Summary", s.intro, (d, v) => { d.home.scorecard.intro = v; }, true)}<div className="inspector-grid">{text("Result value", s.resultValue, (d, v) => { d.home.scorecard.resultValue = v; })}{text("Result unit", s.resultUnit, (d, v) => { d.home.scorecard.resultUnit = v; })}</div>{text("Result label", s.resultLabel, (d, v) => { d.home.scorecard.resultLabel = v; })}{text("Focus label", s.focusLabel, (d, v) => { d.home.scorecard.focusLabel = v; })}{text("Focus value", s.focusValue, (d, v) => { d.home.scorecard.focusValue = v; })}<div className="inspector-grid">{text("Button text", s.buttonLabel, (d, v) => { d.home.scorecard.buttonLabel = v; })}{text("Button link", s.buttonHref, (d, v) => { d.home.scorecard.buttonHref = v; })}</div></>; }
  ...
  if (id === "results-scorecard") { const sc = r.scorecard; const ops = listOps((d) => d.results.scorecard.rows); return <>{text("Eyebrow", sc.eyebrow, (d, v) => { d.results.scorecard.eyebrow = v; })}{text("Heading", sc.heading, (d, v) => { d.results.scorecard.heading = v; })}<p className="inspector-device-note">Column headers</p><div className="inspector-grid">{text("Measure", sc.headerMeasure, (d, v) => { d.results.scorecard.headerMeasure = v; })}{text("Goal", sc.headerGoal, (d, v) => { d.results.scorecard.headerGoal = v; })}{text("Col 3", sc.headerApr, (d, v) => { d.results.scorecard.headerApr = v; })}{text("Col 4", sc.headerMay, (d, v) => { d.results.scorecard.headerMay = v; })}{text("Col 5", sc.headerJun, (d, v) => { d.results.scorecard.headerJun = v; })}</div><p className="inspector-device-note">Rows</p><div className="inspector-list">{sc.rows.map((row, index) => <ListItemFrame key={index} index={index} count={sc.rows.length} onMove={(dir) => ops.move(index, dir)} onRemove={() => ops.remove(index)}><TextField label="Measure" value={row.label} onChange={ops.field(index, (it, v) => { it.label = v; })} /><div className="inspector-grid"><TextField label="Goal" value={row.goal} onChange={ops.field(index, (it, v) => { it.goal = v; })} /><TextField label="Apr" value={row.april} onChange={ops.field(index, (it, v) => { it.april = v; })} /><TextField label="May" value={row.may} onChange={ops.field(index, (it, v) => { it.may = v; })} /><TextField label="Jun" value={row.june} onChange={ops.field(index, (it, v) => { it.june = v; })} /></div></ListItemFrame>)}</div><button type="button" className="list-add" onClick={() => ops.add({ label: "New measure", goal: "", april: "", may: "", june: "" })}>+ Add row</button></>; }
...
export function Editor({ initialDraft, initialPublished, initialRevision, userEmail }: { initialDraft: NewsletterContent; initialPublished: NewsletterContent; initialRevision: number; userEmail: string }) {
  ...
  return <div className="builder">
    <header className="builder-toolbar"><div className="builder-brand"><strong>Newsletter builder</strong><span>{userEmail}</span></div><select aria-label="Page" value={page} onChange={(event) => { setPage(event.target.value as VisualPageId); setSelectedIds([]); setDrawer(null); }}>{pages.map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select><div className="toolbar-group toolbar-group--content"><button type="button" className="toolbar-cmdk" title="Command palette (⌘K)" onClick={() => setPaletteOpen(true)}>⌘K</button><button type="button" className={drawer === "add" ? "is-active" : ""} onClick={() => setDrawer(drawer === "add" ? null : "add")}>＋ Add item</button><button type="button" onClick={() => setDrawer(drawer === "layers" ? null : "layers")}>View</button><button type="button" className={drawer === "design" ? "is-active" : ""} onClick={() => setDrawer(drawer === "design" ? null : "design")}>Design</button><button type="button" className={drawer === "blocks" ? "is-active" : ""} onClick={() => setDrawer(drawer === "blocks" ? null : "blocks")}>Blocks</button><button type="button" className={drawer === "media" ? "is-active" : ""} onClick={() => setDrawer(drawer === "media" ? null : "media")}>Media</button><button type="button" className={drawer === "history" ? "is-active" : ""} onClick={() => setDrawer(drawer === "history" ? null : "history")}>History</button></div><div className="device-switch mode-switch"><button type="button" className={mode === "weekly" ? "is-active" : ""} onClick={() => { setMode("weekly"); setDrawer("weekly"); }}>Weekly</button><button type="button" className={mode === "design" ? "is-active" : ""} onClick={() => { setMode("design"); setDrawer(null); }}>Design mode</button></div><div className="device-switch"><button type="button" className={device === "phone" ? "is-active" : ""} onClick={() => setDevice("phone")}>Phone</button><button type="button" className={device === "desktop" ? "is-active" : ""} onClick={() => setDevice("desktop")}>Desktop</button></div><div className="toolbar-group toolbar-group--undo"><button type="button" title={historyState.undoLabel ? `Undo ${historyState.undoLabel} (\u2318Z)` : "Undo (\u2318Z)"} aria-label="Undo" disabled={!historyState.canUndo || busy} onClick={undo}>↶</button><button type="button" title={historyState.redoLabel ? `Redo ${historyState.redoLabel} (\u2318\u21e7Z)` : "Redo (\u2318\u21e7Z)"} aria-label="Redo" disabled={!historyState.canRedo || busy} onClick={redo}>↷</button></div><span className={`save-state${conflict ? " save-state--error" : ""}`}>{saving ? "Saving…" : dirty ? "Autosave pending" : status} <small>r{revision}</small></span><a className="toolbar-link" href={previewHref} target="_blank">Preview</a><button type="button" onClick={saveNow} disabled={busy || conflict}>Save</button><button type="button" className="publish-button" onClick={() => setChecklistOpen(true)} disabled={busy || conflict}>Publish</button><button type="button" aria-label="Toggle inspector" onClick={() => (isMobile ? setSheetOpen(!sheetOpen) : setInspectorOpen(!inspectorOpen))}>☰</button></header>
