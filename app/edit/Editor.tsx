...
function IconPositionControls({ name, device, layout, patch }: { name: string; device: "phone" | "desktop"; layout?: ResponsiveLayout; patch: (patch: Partial<ResponsiveLayout>) => void }) {
  return <details className="inspector-section" open><summary>{name} icon · {device}</summary><div className="inspector-grid"><NumberField label="Move icon X" value={layout?.iconX ?? 0} min={-80} max={80} onCommit={(value) => patch({ iconX: value ?? 0 })} /><NumberField label="Move icon Y" value={layout?.iconY ?? 0} min={-80} max={80} onCommit={(value) => patch({ iconY: value ?? 0 })} /></div><NumberField label="Icon size %" value={layout?.iconScale ?? 100} min={40} max={180} onCommit={(value) => patch({ iconScale: value ?? 100 })} /><div className="inspector-actions"><button type="button" onClick={() => patch({ iconX: 0, iconY: 0, iconScale: 100 })}>Reset icon</button></div><p className="inspector-device-note">Move and resize this icon exactly where you want it. Phone and desktop can be different.</p></details>;
}
