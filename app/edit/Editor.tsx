...
function NativeEditor({ id, content, change, device, iconLayout, patchIcon }: { id: string; content: NewsletterContent; change: (mutator: (draft: NewsletterContent) => void) => void; device: "phone" | "desktop"; iconLayout?: ResponsiveLayout; patchIcon: (patch: Partial<ResponsiveLayout>) => void }) {
  ...
  if (id === "home-referral") return <>{text("Referral heading", h.grow.referralStrong, (d, v) => { d.home.grow.referralStrong = v; })}{text("Referral detail", h.grow.referralRest, (d, v) => { d.home.grow.referralRest = v; }, true)}<IconPositionControls name="Present" device={device} layout={iconLayout} patch={patchIcon} /></>;
